// Modified from package: https://github.com/geongeorge/Canvas-Txt by geongeorge
...
const CanvasTxt = {
   ...
   * @param {string} mytext
   * @param {boolean} strokeMode
   * @param {number} x
   * @param {number} y
   * @param {number} width
   * @param {number} height
   */
  drawText: function (ctx, mytext, strokeMode = false, x, y, width, height) {
    ...
    textarray.forEach((txtline) => {
      ...
      if (strokeMode) ctx.strokeText(txtline, textanchor, txtY);
      else ctx.fillText(txtline, textanchor, txtY);
      txtY += charHeight;
    });
